# Source

- **Upstream:** https://github.com/NicholasSpisak/claude-code-subagents/tree/main/agents/growth-revenue-operations
- **Resolved repo:** NicholasSpisak/claude-code-subagents
- **Path:** agents/growth-revenue-operations
- **Commit:** 1a16f36820d5db5516cf188712fcbb10ae63811c
- **License:** none declared
- **Mirrored:** 2026-07-27
- **Normalized:** frontmatter `name` changed from `customer-acquisition-gr` to `customer-acquisition-agent`
- **Note:** upstream publishes this as customer-acquisition-gr.md, mirrored as SKILL.md
- **Note:** quoted invalid YAML frontmatter value(s): description

Mirrored into the VCSkills library. Content belongs to its original authors;
see LICENSE-UPSTREAM where present. To request removal, open an issue on
https://github.com/Overdrive-Consulting/vcskills.
